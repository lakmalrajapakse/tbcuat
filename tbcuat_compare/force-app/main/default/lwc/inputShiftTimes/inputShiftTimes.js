...
/**
 * This component depends on moment library loaded
 * in the parent/grand parent component(s)
 */
export default class InputShiftTimes extends LightningElement {
    ...
    _TZ_24_HOUR_TIME_FORMAT = 'HH:mm';
    _TZ_12_HOUR_TIME_FORMAT = 'hh:mm a';
    ...
    setTimes(timeType) {
        ...
        if (this.isReadOnly) {
            ...
            if (this.startTimeValue) {
                ...
                this.tzStartTime = moment(this.startTimeValue).tz(this.siteTimezone);
                this.shiftDate = this.tzStartTime.format(this._TZ_DATE_FORMAT);
                this.startTime = this.tzStartTime.format(this._TZ_12_HOUR_TIME_FORMAT);
                this.dayOfWeek = this.tzStartTime.format('dddd');
            } else {
                this.shiftDate = 'N/A';
                this.startTime = 'N/A';
            }
            if (this.endTimeValue) {
                ...
                this.tzEndTime = moment(this.endTimeValue).tz(this.siteTimezone);
                this.endTime = this.tzEndTime.format(this._TZ_12_HOUR_TIME_FORMAT);
            } else {
                this.endTime = 'N/A';
            }
        } else {
    ...
    parseTimeFromString(strTime, isDT, customFormat) {
        ...
        return parsedTime.format(this._TZ_12_HOUR_TIME_FORMAT);
    }
    ...
    convertTimesToDateTimes() {
        debug('input shift times st: ', this.startTime);
        ...
        // const scheduledStart = new Date(templateStartTime);
        ...
        // const actualStartDayBefore = new Date(actualStart).setDate(actualStart.getDate() - 1);
        // const actualStartDayAfter = new Date(actualStart).setDate(actualStart.getDate() + 1);
        ...
        if (Math.abs(actualStartDayBefore.diff(scheduledStart)) < Math.abs(actualStart.dif(scheduledStart))) {
            actualStart = actualStartDayBefore;
        }

        // Select the day after when it is closer to the scheduled start
        if (Math.abs(actualStartDayAfter.dif(scheduledStart)) < Math.abs(actualStart.dif(scheduledStart))) {
        ...
        actualEnd.hours(actualEndHours);
        actualEnd.minutes(actualEndMinutes);
        actualEnd.seconds(0);
    ...
    reportToParent() {
        ...
        // convert back from site's timezone to utc timezone so times can be stored in salesforce
