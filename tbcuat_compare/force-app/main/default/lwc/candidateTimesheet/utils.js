    ...
    static convertShifts(records = [], siteTimezones, shiftSignatures = {},CustomExternalIDForShiftRecords, DefaultTimeZone)
    {
        ...
        for(let record of records){
            ...
            let shift = {
                ...
                approvedStartDate: hasApprovedStart ? Utils.convertDateFormat(new Date(record?.sirenum__Scheduled_Start_Time__c)) : '',
                approvedEndDate: hasApprovedEnd ? Utils.convertDateFormat(new Date(record?.sirenum__Scheduled_End_Time__c)) : '',
                approvedStartTime: hasApprovedStart ? Utils.convertTimeFormat(new Date(record?.sirenum__Scheduled_Start_Time__c)) : '',
                approvedEndTime: hasApprovedEnd ? Utils.convertTimeFormat(new Date(record?.sirenum__Scheduled_End_Time__c)) : '',
                ...
                approvedStart: hasApprovedStart ? record?.sirenum__Scheduled_Start_Time__c : (hasActualStart ? record?.sirenum__Actual_Start_Time__c : record?.sirenum__Scheduled_Start_Time__c),
                approvedEnd: hasApprovedEnd ? record?.sirenum__Scheduled_End_Time__c : (hasActualEnd ? record?.sirenum__Actual_End_Time__c : record?.sirenum__Scheduled_End_Time__c),
                ...
                isPayrolled: !!record?.sirenum__Timesheet_summaries__c,
            ...
            console.log('expenses: ',record);
            console.log('expensess: ',this.hasExpenses);
            ...
            if (shift.portalHoursEntry) {
                ...
                let startTime = record?.sirenum__Actual_Start_Time__c ?? defaultStart;
                let endTime = record?.sirenum__Actual_End_Time__c ?? defaultEnd;
    ...
    static convertExpenses(expenses = []) {
        ...
        for (let expense of expenses) {
            let newExpense = {
                ...
                'ExpenseTypeValue': expense?.sirenum__Expense_Type__r?.sirenum__Value__c ?? '£0.00',
                'Amount': expense?.Expense_value__c ?? '£0.00',
                'Units': expense?.sirenum__Units__c ?? '£0.00',
